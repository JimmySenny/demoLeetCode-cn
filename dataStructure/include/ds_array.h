#include "ds.h"

/* Array */
//一维数组
extern int pivotIndex( int* nums, int numsSize );
extern int dominantIndex(int* nums, int numsSize);
extern int* plusOne(int* digits, int digitsSize, int* returnSize);
extern int searchInsert(int* nums, int numsSize, int target);
extern int searchInsert1(int* nums, int numsSize, int target);
extern int** merge(int** intervals, int intervalsSize, int* intervalsColSize, int* returnSize, int** returnColumnSizes);
//二维数组
extern void setZeroes(int** matrix, int matrixSize, int* matrixColSize);
extern int* findDiagonalOrder(int** matrix, int matrixSize, int* matrixColSize, int* returnSize);

//数组相关技术
extern int** generate( int numRows, int* returnSize, int** returnColumSizes );
extern int* getRow( int rowIndex, int* returnSize );
extern char* reverseWords( char * s );
extern int findMin( int* nums, int numsSize );
